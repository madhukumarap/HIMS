...
router.get("/getLastCreatedHospital", async (req, res) => {
  ...
  try {
    ...
    // Only try to read the image if the path exists
    if (imagePath) {
      try {
        const imageBuffer = fs.readFileSync(imagePath);
        imageBase64 = imageBuffer.toString("base64");
        ...
        // You might want to handle this differently - maybe set a default image?
      }
    }
    ...
    const hospitalWithImage = {
      ...
      logo: imageBase64, // This will be null if no image was found
    };
