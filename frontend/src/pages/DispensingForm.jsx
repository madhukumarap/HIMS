...
useEffect(() => {
  if (currency && exchangeRates) {
    ...
    const totalFeesInSelectedCurrency = orders.reduce((sum, order) => {
      const nativeCurrency = (order.item.Currency || "USD").toUpperCase().replace(/[^A-Z]/g, "");
      const selectedCurrency = currency.toUpperCase().replace(/[^A-Z]/g, "");

      const nativeRate = exchangeRates[nativeCurrency] ?? 1;
      const selectedRate = exchangeRates[selectedCurrency] ?? 1;

      console.log(`Rates for ${nativeCurrency} → ${selectedCurrency}:`, {
        nativeRate,
        selectedRate,
      });

      // Skip if unitPrice is invalid
      if (isNaN(order.item.unitPrice)) {
        console.warn("Invalid unitPrice for order:", order);
        return sum;
      }

      // Calculate converted price
      const unitPriceInSelectedCurrency = order.item.unitPrice * (selectedRate / nativeRate);
      const itemTotal = order.quantity * unitPriceInSelectedCurrency;
    ...
    // Handle NaN cases
    const formattedTotalFees = isNaN(totalFeesInSelectedCurrency)
      ? "0.00"
      : totalFeesInSelectedCurrency.toFixed(2);
