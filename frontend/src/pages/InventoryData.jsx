...
function InventoryManagementPage() {
  ...
  if (item.balanceQuantity === 0) return true;
  
  const belowAbsoluteThreshold = item.balanceQuantity < absoluteThreshold;
  
  const belowPercentageThreshold = item.quantityIn > 0 && 
    (item.balanceQuantity / item.quantityIn) < percentageThreshold;

  return belowAbsoluteThreshold || belowPercentageThreshold;
};
                ...
                <Td
                  style={{
                    ...
                    fontWeight: 
                      (needsReorder(item) || 
                       (selectedOption === "monthly" 
                         ? item.AverageMonthlyQuantityOut * multiplier > item.balanceQuantity
                         : item.AverageWeeklyQuantityOut * multiplier > item.balanceQuantity)
                      ) && !isExpired(item.expiryDate)
                        ? "bold"
                        : "inherit",
                    backgroundColor: (needsReorder(item) && !isExpired(item.expiryDate)) || 
                                    (item.balanceQuantity === 0 && !isExpired(item.expiryDate))
                      ? "#FFF3CD" 
                      : "inherit"
                  }}
                >
