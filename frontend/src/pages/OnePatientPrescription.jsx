...
function PrescriptionDetails(props) {
  ...
  const sendReminder = (id) => {
    axios
      .post(
        `${import.meta.env.VITE_API_URL}/api/sendReminder/${id}`,
        {},
        {
          headers: {
            Authorization: `${currentUser?.Token}`,
      ...
      .then((response) => {
        toast.success(t("ReminderEmailSentSuccessfully"));
      })
      .catch((error) => {
        toast.error(t("couldNotSentEmail"));
  ...
  return (
    <>
            ...
            >
              <thead>
                <tr>
                  ...
                  <th style={{ whiteSpace: "nowrap" }}>{t("RevisitDate")}</th>
                  {/*               <th style={{ textAlign: "center" }}>{t("patientPrescriptionTable.ViewDetails")}</th>
                  <th style={{ whiteSpace: "nowrap" }}>
                  ...
                  patient.patientPrecription
                    ...
                    .map((prescription) => (
                      <tr key={prescription.id}>
                        ...
                        <td style={{ textAlign: "center" }}>
                          {prescription.revisitDate
                            ? formatDateInSelectedLanguage(
                                new Date(prescription.revisitDate)
                              )
                            : "NA"}
                        </td>
                        <td className="d-flex align-items-center">
                          ...
                          {prescription.revisitDate ? (
                            <button
                              ...
                              disabled={checkDate(prescription.revisitDate)}
                              onClick={() => sendReminder(prescription.id)}
                              style={{
                                fontSize: "12px",
                                marginTop: "10px",
                                padding: "4px 5px",
                        ...
                        <td style={{ textAlign: "center" }}>
                          {prescription.emailSentStatus == "sent"
                            ? t("Sent")
                            : t("NotSent")}
                        </td>
                        {/* <td style={{ textAlign: "center" }}>
                        ...
                        <td style={{ textAlign: "center" }}>
                          {prescription.emailSentDetails &&
                            prescription.emailSentDetails
                              .split(",")
                              .map((date, index) => (
                                <div key={index}>
                                  {date && isDate(new Date(date))
                                    ? formatDateInSelectedLanguage(
                                        new Date(date)
                                      )
                                    : "Invalid Date"}
      ...
      <Modal centered style={style} show={showModal} onHide={handleCloseModal}>
