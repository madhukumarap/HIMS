...
function AddResultModalForBloodSugarForFastingTest({
  showFieldModal,
  ...
  const fetchColumns = async (selectedTest) => {
    ...
    const tableName =
      selectedTest.toLowerCase().replace(/\s+/g, "") + "resultmodels"; // Generate tableName
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const tableName =
        selectedTest?.toLowerCase().replace(/\s+/g, "") + "resultmodels";
      ...
      // return;
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/insertTestData`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
...
export default AddResultModalForBloodSugarForFastingTest;
