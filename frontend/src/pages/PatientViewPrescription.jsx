...
function PrescriptionDetails(props) {
    ...
    async function fetchPatient() {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_URL}/api/getOnePaisentRegPatient/${currentUser.phoneNumber}`,
          {
            headers: {
  ...
  return (
    <>
      ...
      >
        <h2 style={h1Style}>My {t("PrescriptionList")}</h2>
      </header>
      <br></br>
              ...
              patient.patientPrecription
                ...
                .map((prescription) => (
                  <Card key={prescription.id} className="mb-3">
                    <Card.Body>
                      <Card.Title>
                        <>Doctor Name:</> {prescription.PrescribedDoctor}
                      </Card.Title>
                      <Card.Text>
                        <strong> Prescription ID:</strong>{" "}
                        {prescription.prescriptionId}
                        <br />
                        <strong>Reg No:</strong> {prescription.RegistrationNo}
                        <br />
                        <strong>Phone No:</strong> {prescription.PhoneNo}
                        <br />
                        ...
                        <div className="d-flex justify-content-center">
                          <Link
                            to={`/${extractedPart}/PatientRoleMedecines/${patient.id}/${prescription.id}`}
                          >
                            <button
                          ...
                          <Link
                            to={`/${extractedPart}/PatientMedicationReport/${prescription.id}`}
                          >
                            <button
                              title="View Medication Report"
                              style={{
                                fontSize: "12px",
                                marginTop: "10px",
                                marginLeft: "10px",
          ...
          <div style={style} className="table-responsive">
            <Table
              style={{
                ...
                margin: "0 auto" /* Optional: Centers the page horizontally */,
                fontSize:
                  "12px" /* Adjust the font size as per your requirement */,
                verticalAlign: "middle",
                textAlign: "center",
              }}
              responsive
              className="table table-bordered"
              hover // Add hover attribute to enable hover effect
            >
              <thead>
                <tr>
                  ...
                  <th className="text-center nowrip">{t("DoctorName")}</th>
                  ...
                  <th className="text-center nowrip">Action</th>
                  {/* <th className="text-center nowrip">Update Medication</th>
              <th className="text-center nowrip">View Medication</th> */}
                </tr>
              </thead>
              <tbody>
                {patient.patientPrecription &&
                  patient.patientPrecription
                    ...
                    .map((prescription) => (
                      <tr
                        key={prescription.id}
                        style={rowHoverStyle} // Apply the hover style here
                      >
                        <td className="text-center nowrip">
                          {prescription.prescriptionId}
                        </td>
                        <td className="text-center nowrip">{`${prescription.PrescribedDoctor} `}</td>
                        <td className="text-center nowrip">
