...
const PerTestRevenueDiagnostic = () => {
    ...
    if (!selectedTestID) {
      axios
        .get(
          ...
          }/api/getTestCountsAndRevenueDiagnostic`,
          {
            params: {
              startDate,
              endDate,
  ...
  const [DiagnosticsTests, setDiagnosticsTests] = useState([]);
  const getTestName = (selectedTestID) => {
    const selectedTest = DiagnosticsTests?.find(
      (test) => test.id === parseInt(selectedTestID)
    );
    return selectedTest;
  };
  useEffect(() => {
    // Fetch all Diagnostics tests
    axios
      .get(`${import.meta.env.VITE_API_URL}/api/getAllDiagnosticTests`)
      .then((response) => {
        console.log("Test Data" + JSON.stringify(response.data?.tests));
        setDiagnosticsTests(response.data?.tests);
      })
      .catch((error) => {
        console.error("Error fetching Diagnostics tests:", error);
      });
  }, []);
  ...
  const fetchData = () => {
    ...
    const url = `${
      import.meta.env.VITE_API_URL
    }/api/getSelectedTestRevenueDiagnostic`;
  ...
  return (
    ...
    >
      ...
      >
        <h2 style={{ fontSize: "18px" }}>Per Test Revenue(Diagnostics)</h2>
      </header>
      <br></br>
...
export default PerTestRevenueDiagnostic;
