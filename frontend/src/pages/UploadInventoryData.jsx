...
import sampleCsvFile from "../assets/InventorySampleData.csv";
...
const UploadCsvFileNurse = () => {
  ...
  const handleDownloadSampleFile = () => {
    ...
    link.download = "InventorySampleData.csv";
    link.click();
  };
  const handleFileChange = (event) => {
    ...
    if (allowedExtensions.exec(selectedFile.name)) {
      ...
      toast.error(t("Onlycsvfilesareallowed"));
    }
  };
  ...
  const handleSubmit = async (event) => {
    ...
    if (file) {
      try {
        ...
        await axios.post(
          `${import.meta.env.VITE_API_URL}/api/UploadInventoryData`,
          formData,
          {
            headers: {
        ...
        toast.success(t("InventoryDataSavedSuccessfullyIfNotPresentData"));

        navigate(`/${extractedPart}/EditInventory`);
      } catch (error) {
      ...
      >
        ...
        <Modal.Body>
          <h3 className="text-center">
            {t(
              "PleaseClickOnDownloadIconAndReferSampleCsvDataToAddMultipleMedicineData"
            )}
          </h3>
        </Modal.Body>
        <Modal.Footer>
